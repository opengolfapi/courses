...
export function SubmissionButtons({ submissionId }: { submissionId: string }) {
  ...
  function handleApprove() {
    startTransition(async () => {
      try {
        ...
        const result = await approveSubmission(submissionId, token);
  ...
  function handleReject() {
    ...
    startTransition(async () => {
      try {
        const token = await getToken();
        if (!token) { setErr('Not signed in — refresh the page'); return; }
        const result = await rejectSubmission(submissionId, reason, token);
        if (result && 'ok' in result && !result.ok) {
          setErr(result.error || 'unknown');
          return;
        }
